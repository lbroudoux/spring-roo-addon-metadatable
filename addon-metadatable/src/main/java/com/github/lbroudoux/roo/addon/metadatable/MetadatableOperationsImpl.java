...
@Service
public class MetadatableOperationsImpl implements MetadatableOperations {
	...
	public void annotateType(JavaType javaType) {
		...
		if (existing != null && MemberFindingUtils.getAnnotationOfType(existing.getAnnotations(), new JavaType(RooMetadatable.class.getName())) == null) {
			...
			JavaType rooRooMetadatable = new JavaType(RooMetadatable.class.getName());

			// Create Annotation metadata
			AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooMetadatable);
	...
	public void setup() {
      ...
      //projectOperations.addRepository("", new Repository("Metadatable Roo add-on repository", "Metadatable Roo add-on repository", "https://com-github-lbroudoux-roo-addon-metadatable.googlecode.com/svn/repo"));
		
		// Install the dependency on the add-on jar (
		dependencies.add(new Dependency("com.github.lbroudoux.roo.addon", "com.github.lbroudoux.roo.addon.metadatable", "0.1.0.BUILD-SNAPSHOT", DependencyType.JAR, DependencyScope.PROVIDED));
